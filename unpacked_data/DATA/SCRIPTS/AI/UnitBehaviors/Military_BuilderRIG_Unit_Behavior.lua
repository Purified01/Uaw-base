...
function AI_Behavior_Update()
	...
	elseif TestValid( CommandCenter ) and Object.Get_Distance( CommandCenter ) > Novus_Constructor.HEAL_RANGE then
		...
		if TestValid(MostDamagedObject) then
			...
			if Object.Get_Distance( MostDamagedObject ) < Novus_Constructor.ABILITY_RANGE * 1.5 and not MostDamagedObject.Is_Category("Organic") then -- MW: Only Heal Machines and Structures
				Object.Activate_Ability(Novus_Constructor.HEAL_ABILITY,true,MostDamagedObject)
				Clear_Info_And_Sleep(2.0)
				return
			elseif TestValid( CommandCenter ) and MostDamagedObject.Get_Distance( Object ) < Novus_Constructor.HEAL_RANGE and not MostDamagedObject.Is_Category("Organic") then
