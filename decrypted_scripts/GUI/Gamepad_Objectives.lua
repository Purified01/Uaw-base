...
--/////////////////////////////////////////////////////////////////////////////////////////////////
--
--              File: //depot/Projects/Invasion_360/Run/Data/Scripts/GUI/Gamepad_Objectives.lua
--
...
function On_Init()
	...
	for index, text in pairs(texts) do
		...
		total_text_height = total_text_height + text_bounds.h
		
		this.Register_Event_Handler("Component_Unhidden", text, On_Objective_Shown)
		this.Register_Event_Handler("Animation_Finished", text, Objective_Animation_Finished)
	end
	
	local _, frame_y, _, frame_h = this.Frame.Get_World_Bounds()
	FrameMargin = 2.5*(FrameMargin - frame_y)
	
	-- ARTISTS CAN USE THIS VALUES TO TUNE HOW LONG THE CHANGE OF COLOR TAKES
	-- AND WHAT THE FLASHY AND DEFAULT COLORS SHOULD BE!!!!!
	-- -------------------------------------------------------------------------------------
	OBJECTIVES_TEXT_COLOR_FADE_DURATION = 2.0 
...
-- ---------------------------------------------------------------------------------------------------------------------------
function On_Objectives_Changed(_, _, adding, remove_obj_at_idx)
	...
	if #sort_table <= 0 then
		...
		PlayRevealAnim = {}
		FlashingText = {}
		this.Frame.Set_Hidden(true)
	else
		this.Frame.Set_Hidden(false)
	end
	...
	for _, index in pairs(sort_table) do
		...
		if objective then
			...
			if adding == true and gui.text.Get_Hidden() then
				PlayRevealAnim[gui.text] = true
			end
	...
	local f_x, f_y, f_w, _ = this.Frame.Get_World_Bounds()
	this.Frame.Set_World_Bounds(f_x, f_y, f_w, total_text_height + FrameMargin)
...
function On_Update(event, source)
	if Showing then
		...
		for text, anim_data in pairs(FlashingText) do
			if not anim_data.Done and anim_data.AnimStartTime + OBJECTIVES_TEXT_COLOR_FADE_DURATION <= curr_time then
				Stop_Flash(text)
				anim_data.Done = true
			end		
		end
...
function On_Objective_Shown(_, objective_text)
	if PlayRevealAnim[objective_text] then
		objective_text.Play_Animation("Slide_In", false)
	end
end

function Objective_Animation_Finished(_, objective_text)
	if PlayRevealAnim[objective_text] then
		local x, _, w, movie_h = this.RevealMovie.Get_World_Bounds()
		local _, y, _, _ = objective_text.Get_World_Bounds()
		this.RevealMovie.Set_World_Bounds(x, y, w, movie_h)	
		this.RevealMovie.Restart()	
		
		objective_text.Stop_Animation()
		Start_Flash(objective_text)
		FlashingText[objective_text] = {AnimStartTime = GetCurrentTime()}
		PlayRevealAnim[objective_text] = nil
	end
end
